...
export function Hero(){
 ...
 useEffect(()=>{const ctx=gsap.context(()=>{gsap.fromTo('.hero-enter',{opacity:0,y:28},{opacity:1,y:0,duration:1,stagger:.12,delay:.15,ease:'power4.out'});},ref);return()=>ctx.revert();},[]);
 return <section ref={ref} id="home" className="min-h-screen flex items-center pt-28 pb-16">
  <div className="page-shell w-full grid lg:grid-cols-[1.15fr_.85fr] gap-12 lg:gap-4 items-center">
   <div className="hero-enter">
    ...
    <div className="flex flex-wrap items-center gap-5 mt-9">
      <a href="#projects" className="primary-pill">Explore work <ArrowUpRight size={15} className="ml-2"/></a>
      <a href="#contact" className="ghost-link">Let's talk →</a>
    </div>
    <div className="flex flex-wrap gap-10 mt-16 pt-6 border-t border-white/10 max-w-xl">
      <div><p className="text-2xl font-light">10+</p><p className="section-label !text-white/40 !text-[9px] mt-1">Projects</p></div>
      <div><p className="text-2xl font-light">AI/ML</p><p className="section-label !text-white/40 !text-[9px] mt-1">Focus</p></div>
      <div><p className="text-2xl font-light text-[var(--verdant)]">Open</p><p className="section-label !text-white/40 !text-[9px] mt-1">For opportunities</p></div>
    </div>
   </div>
