  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.hero-meta', { opacity: 0, y: -16 }, { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out' })
        ...
        .fromTo('.hero-image-panel', { opacity: 0, scale: 0.92 },
          { opacity: 1, scale: 1, duration: 1.2, ease: 'expo.inOut' }, '-=0.8');
    }, containerRef);
    ...
    <section ref={containerRef} id="home" className="min-h-screen flex flex-col justify-center pt-24 pb-12 relative overflow-visible">
      <div className="container mx-auto px-6 md:px-12 relative z-10 flex-1 flex flex-col">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 flex-1 pb-8">
          ...
          {/* Right Column — Lanyard, hanging free (no panel box) */}
          <div className="lg:col-span-5 hero-image-panel opacity-0 relative overflow-visible">
            <div className="relative w-full h-[420px] md:h-[520px] overflow-visible">
              ...
              {/* Unboxed 3D lanyard card. Stretched well above its footprint and left
                  unclipped so the band itself appears to drop from the top of the page
                  rather than being confined to a bordered panel. */}
              <div className="absolute inset-x-0 bottom-0 -top-[200px] sm:-top-[260px] lg:-top-[320px]">
                <CanvasErrorBoundary
                  fallback={
                    <img
                      src={profile1}
              ...
              {/* Hint label, anchored to the card's original footprint */}
