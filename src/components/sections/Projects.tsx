    ...
    <div className="page-shell">
      ...
      <div className="border-t border-white/15">
        {portfolioData.projects.map((project,index)=><article key={index} className="project-card group grid md:grid-cols-[80px_1fr_auto] gap-5 md:gap-10 items-start py-9 border-b border-white/10">
          ...
          <div className="flex gap-2 md:pt-1">{project.githubUrl&&<a href={project.githubUrl} target="_blank" rel="noopener noreferrer" aria-label={`${project.title} GitHub`} className="w-9 h-9 rounded-full border border-white/15 flex items-center justify-center text-white/55 hover:text-white hover:border-white/40 transition-colors"><FolderGit2 size={14}/></a>}{project.liveUrl&&project.liveUrl!=='#'&&<a href={project.liveUrl} target="_blank" rel="noopener noreferrer" aria-label={`${project.title} live site`} className="w-9 h-9 rounded-full border border-white/15 flex items-center justify-center text-white/55 hover:text-white hover:border-white/40 transition-colors"><ArrowUpRight size={15}/></a>}</div>
        </article>)}
      </div>
    </div>
  </section>;
